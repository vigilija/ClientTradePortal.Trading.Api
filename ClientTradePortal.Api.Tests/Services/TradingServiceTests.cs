...
public class TradingServiceTests
{
    ...
    public async Task PlaceOrderAsync_ShouldCreateNewPosition_WhenPositionDoesNotExist()
    {
        ...
        // Convert ICollection<StockPosition> to a List to allow indexing
        account.Positions.Should().HaveCount(1);
        var position = account.Positions.FirstOrDefault();
        position.Should().NotBeNull();
        position!.Symbol.Should().Be("MSFT");
        position.Quantity.Should().Be(10);
        position.AveragePrice.Should().Be(stockPrice);
    ...
    public async Task PlaceOrderAsync_ShouldUpdateExistingPosition_WhenPositionExists()
    {
        ...
        position.AveragePrice.Should().BeApproximately(153.33m, 0.01m);
